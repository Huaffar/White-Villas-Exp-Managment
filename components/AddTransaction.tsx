...
const AddTransaction: React.FC<AddTransactionProps> = ({ onAddTransaction, projects, staff, incomeCategories, expenseCategories }) => {
    ...
    return (
        <div className="space-y-8">
            <h1 className="text-3xl font-bold primary-text">Add New Transaction</h1>
            <div className="bg-gray-800 p-8 rounded-lg shadow-2xl w-full max-w-3xl mx-auto border border-gray-700">
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div>
                        <label className="block text-sm font-medium text-gray-300 mb-2">Transaction Type</label>
                        <div className="flex gap-4">
                            <button type="button" onClick={() => setType(TransactionType.EXPENSE)} className={`w-full py-3 rounded-lg font-semibold transition-colors ${type === TransactionType.EXPENSE ? 'bg-red-500' : 'bg-gray-700 hover:bg-gray-600'}`}>Expense</button>
                            <button type="button" onClick={() => setType(TransactionType.INCOME)} className={`w-full py-3 rounded-lg font-semibold transition-colors ${type === TransactionType.INCOME ? 'bg-green-500' : 'bg-gray-700 hover:bg-gray-600'}`}>Income</button>
                        </div>
                    </div>
                    ...
                    {(category === 'Salaries' || category === 'Commission') && type === TransactionType.EXPENSE && (
                        <div>
                    ...
                    <div className="flex justify-end pt-4">
                        <button type="submit" className="px-8 py-3 primary-bg text-gray-900 font-bold rounded-lg hover:opacity-90">Add Transaction</button>
                    </div>
                </form>
            </div>
        </div>
